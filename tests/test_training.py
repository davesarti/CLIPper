...
def _triplets(count=4):
    return [
        Triplet(ref=i, positives=[0], negatives=[1], target=count + i,
                violation=2 * count + i, distractor=3 * count + i)
...
def test_build_batch_gathers_features_and_negatives():
    ...
    # Slots are [violation, distractor, lazy]; the last is the reference itself.
    assert torch.equal(batch.negatives[0, 0], features[triplets[0].violation])
