...
method was arrived at, and what each step measured. The current proposal is
`docs/method-proposal-cpas.md`.
...
`results/attribute_caption_results.csv` and `results/attribute_accuracy.csv`.)
...
proposal, `docs/method-proposal-cpas.md` §4).
...
(`results/cpas_ablation.csv`). "Leakage" is the mean absolute probe drift on the 38
